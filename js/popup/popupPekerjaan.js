var popupPekerjaan = new DaftarObj2({
	prefix : 'popupPekerjaan',
	url : 'pages.php?Pg=popupPekerjaan',
	formName : 'popupPekerjaanForm',
	kodeProgram : 'p',
	namaProgram : 'program',
	filterAkun : '',
	...
	windowShow: function(postPrefix){
		...
		// popupPekerjaan_cover
		$("#"+cover).css("overflow", "auto");


		$.ajax({
			type:'POST',
			data:$('#'+this.formName).serialize()+"&postKategori="+$("#kategoriPekerjaan").val(),
		...
		var me = this;
			$.ajax({
				...
				var resp = eval('(' + data + ')');
					delElem(cover);
					if(resp.err == ''){
						$("#idPekerjaan").val(resp.content.idPekerjaan);
						$("#namaPemda").val(resp.content.namaPemda);
						$("#namaPekerjaan").val(resp.content.namaPekerjaan);
						$("#idPemda").val(resp.content.idPemda);
						me.windowClose();
					}else{
						alert(resp.err);
					}
