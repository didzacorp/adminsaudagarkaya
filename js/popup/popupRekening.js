var popupRekening = new DaftarObj2({
	...
	windowShow: function(postPrefix){
		...
		var postTambahan = "";
		if(postPrefix.postFilterK != ''){
			postTambahan = "&postFilterK="+postPrefix.postFilterK;
		}
		if(postPrefix.postFilterL != ''){
			postTambahan = postTambahan+"&postFilterL="+postPrefix.postFilterL;
		}
		$.ajax({
			type:'POST',
				...
				var resp = eval('(' + data + ')');
					...
					if(resp.err == ''){
						...
						if($("#kategoriPekerjaan")){
							$("#kategoriPekerjaan").val(resp.content.kategoriPekerjaan);
						}
