...
class GenerateJSON extends Config{
  ...
  function __construct(){
    ...
    for ($iZX=0; $iZX < sizeof($arrayKodeSKPD); $iZX++) {
      ...
      $getDataRekap = $this->sqlArray($this->sqlQuery("select sum(jumlah_baik) as jumlah_baik,sum(jumlah_kurang_baik) as jumlah_kurang_baik,sum(jumlah_rusak_berat) as jumlah_rusak_berat,sum(jumlah_tidak_ada) as jumlah_tidak_ada  from rekap_inventaris where   c1 = '".$explodeKodeSKPD[0]."' and c = '".$explodeKodeSKPD[1]."' and d = '".$explodeKodeSKPD[2]."' and e = '".$explodeKodeSKPD[3]."' and e1 = '".$explodeKodeSKPD[4]."' and tahun='".$getSettingTahunInventaris['nilai']."'  "));
      $rekapSudahInventaris = $getDataRekap['jumlah_baik'] + $getDataRekap['jumlah_kurang_baik'] + $getDataRekap['jumlah_rusak_berat'] + $getDataRekap['jumlah_tidak_ada'];
      ...
      if($rekapSudahInventaris != $getJumlahSudahInventaris['count(idbi)']){
        file_put_contents('listSKPD.txt', $kodeSKPD .PHP_EOL , FILE_APPEND | LOCK_EX);
      echo "\033[0;31m".$kodeSKPD." => ".$this->numberFormat($rekapSudahInventaris)." || ".$this->numberFormat($getJumlahSudahInventaris['count(idbi)'])." ERROR "."\033[0m"." \n";
      }else{
        echo  "KODE SKPD : $kodeSKPD"." => ".$this->numberFormat($rekapSudahInventaris)." || ".$this->numberFormat($getJumlahSudahInventaris['count(idbi)'])."\n";
      }
