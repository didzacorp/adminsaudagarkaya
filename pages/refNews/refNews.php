...
class refNewsObj  extends configClass{
	...
	var $PageTitle = 'refNews';
	var $PageIcon = 'images/administrasi_ico.png';
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='refNews.xls';
	var $namaModulCetak='refNews';
	var $Cetak_Judul = 'refNews';
	var $Cetak_Mode=2;
	  ...
	  if(empty($judulNews)) {
			...
			if($fmST == 0){
				if($err==''){
							$dataInsert = array(
								'judul' => $judulNews,
                'isi_content' => base64_encode($deskripsiNews),
                ...
                'status' => $statusNews,
							...
							$queryInsert = $this->sqlInsert('news',$dataInsert);
							$cek = $queryInsert;
				...
				if($err==''){
					...
					$queryUpdate = $this->sqlUpdate('news',$dataUpdate,"id = '".$idplh."'");
...
".
	  ...
	  if ($this->form_fmST==0) {
		...
		$this->form_fields = array(
			'judul' => array(
						...
						'labelWidth'=>2,
						'contentWidth'=>10,
						'value'=>"<input type='text' name = 'judulNews' id = 'judulNews' class='form-control form-control-sm' value='$judul' >",
						 ),
			'sourceVideo' => array(
						'label'=>'SOURCE VIDEO',
						'labelWidth'=>2,
						'contentWidth'=>10,
						'value'=>"<input type='text' name = 'sourceVideo' id = 'sourceVideo' class='form-control form-control-sm' value='$source_video' >",
						 ),
		 'kategori' => array(
					'label'=>'KATEGORI',
					'labelWidth'=>2,
					'contentWidth'=>2,
					'value'=> cmbArray("kategoriNews",$kategori,$arrayKategori,"-- KATEGORI --","class='form-control form-control-sm'"),
					 ),
		 'deskripsi' => array(
					 'label'=>'DESKRIPSI',
					 'labelWidth'=>2,
					 'contentWidth'=>10,
					 // 'type' => 'manual',
					 'value'=>"<div id ='deskripsiNews' class='quill-container' style='height:300px;' >$deskripsiProduk</div>",
						),
	 			'status' => array(
						'label'=>'STATUS',
						'labelWidth'=>2,
						'contentWidth'=>2,
						'value'=> cmbArray("statusNews",$status,array(array("AKTIF","AKTIF"),array("TIDAK AKTIF","TIDAK AKTIF")),"-- STATUS --","class='form-control form-control-sm'"),
				 ),
			);
		//tombol
		$this->form_menubawah =
	...
	function bayarKomisi($idrefNews){
	 global $SensusTmp;
	 $cek = ''; $err=''; $content='';
	 $json = TRUE;	//$ErrMsg = 'tes';
	 $form_name = $this->Prefix.'_form';
	 ...
	 $this->form_height = 175;
			...
			$getData = $this->sqlArray($this->sqlQuery("select * from refNews where id = '$idrefNews'"));
			foreach ($getData as $key => $value) {
	 			 $$key = $value;
	 		 }
			 $jenisrefNews = $type_refNews;
	 ...
	 //items ----------------------
	  $this->form_fields = array(
		...
		//tombol
		$this->form_menubawah =
			"<input type='button' class='btn btn-success' value='Simpan' onclick ='".$this->Prefix.".savePembayaranKomisi($idrefNews)' title='Simpan'>&nbsp&nbsp".
	...
	function broadcastEmail(){
	 ...
	 $this->form_height = 175;
			...
			$listEmail = implode(';',$_REQUEST['refNews_cb']);
	 ...
	 //items ----------------------
	  $this->form_fields = array(
		...
		//tombol
		$this->form_menubawah =
			"<input type='button' value='Simpan' onclick ='".$this->Prefix.".sendBroadCast($idrefNews)' title='Simpan'>&nbsp&nbsp".
	...
	function addTestimoni($idrefNews){
	 global $SensusTmp;
	 $cek = ''; $err=''; $content='';
	 $json = TRUE;	//$ErrMsg = 'tes';
	 $form_name = $this->Prefix.'_form';
	 ...
	 //items ----------------------
	  $this->form_fields = array(
		...
		//tombol
		$this->form_menubawah =
			"<input type='button' value='Simpan' class='btn btn-success' onclick ='".$this->Prefix.".saveTestimoni($idrefNews)' title='Simpan'>&nbsp&nbsp".
	  ...
	  "<thead>
	   <tr>
  	   ...
  	   $Checkbox
		   ...
		   <th class='th01' style='text-align:center;vertical-align:middle;width:20%;'>SOURCE VIDEO</th>
	...
	function getDaftarOpsi($Mode=1){
		...
		$arrOrders = array();
		switch($filterUrut){
			case '1': $arrOrders[] = " type_refNews $Asc1 " ;break;
   ...
   $this->form_caption = 'DIREC TEAM';
	 ...
	 $getDataMember = $this->sqlArray($this->sqlQuery("select * from news where id = '$idMember'"));
	...
	function listDirectTeam($idMember){
			...
			$getDataMember = sqlQuery("select * from news where upline_level_1 = '".$idMember."' ");
			$no=1;
