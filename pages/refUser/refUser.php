...
class refUserObj  extends configClass{
	var $Prefix = 'refUser';
	...
	var $PageTitle = 'refUser';
	var $PageIcon = 'images/administrasi_ico.png';
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='refUser.xls';
	var $namaModulCetak='refUser';
	var $Cetak_Judul = 'refUser';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'refUserForm';
	var $noModul=14;
	...
	//get data -----------------
	 ...
	 if (empty($emailUser)) {
			 $err = "Isi Email ";
	 } elseif (empty($statusUser)) {
			 $err = "Pilih Status";
	 }

			if($fmST == 0){
			  if ($this->sqlNumRow($this->sqlQuery("select * from member where email='$emailUser'")) !=0) {
                ...
                'last_update' => date("Y-m-d H:i:s"),
							...
							$queryInsert = $this->sqlInsert('users',$dataInsert);
							$cek = $queryInsert;
				...
				if($err==''){
					...
					$queryUpdate = $this->sqlUpdate('users',$dataUpdate,"id = '".$idplh."'");
	...
	function set_selector_other($tipe){
	 ...
	 function setPage_OtherScript(){
		...
		return
			"<script type='text/javascript' src='js/refUser/refUser.js' language='JavaScript' ></script>
...
".
	...
	function setForm($dt){
	 ...
	 $this->form_height = 270;
	  if ($this->form_fmST==0) {
		...
		$kondisiBaru = " where id not in (select id_member from users) ";
	  }else{
		...
		$this->form_fields = array(
			'emailMember' => array(
						...
						'labelWidth'=>4,
						'contentWidth'=>8,
						'value'=>"<input type='text' name = 'emailUser' id = 'emailUser' class='form-control form-control-sm' value='$email' $readonly2 >",
						 ),
			'password' => array(
						'label'=>'PASSWORD',
						'labelWidth'=>4,
						'contentWidth'=>8,
						'value'=>"<input type='text' name = 'passwordUser' id = 'passwordUser'  class='form-control form-control-sm'  >",
						 ),
	 			'status' => array(
						'label'=>'STATUS',
						'labelWidth'=>4,
						'contentWidth'=>8,
						'value'=> cmbArray("statusUser",$status,array(array("AKTIF","AKTIF"),array("TIDAK AKTIF","TIDAK AKTIF")),"-- STATUS --","class='form-control form-control-sm'"),
				 ),
	 			'idMember' => array(
						'label'=>' MEMBER',
						'labelWidth'=>4,
						'contentWidth'=>8,
						'value'=> cmbQuery('idMember',$id_member,"select id ,nama from member $kondisiBaru" ,"class='form-control form-control-sm' $readonly",'-- MEMBER --'),
				 ),
			);
		//tombol
		$this->form_menubawah =
	...
	function bayarKomisi($idrefUser){
	 global $SensusTmp;
	 $cek = ''; $err=''; $content='';
	 $json = TRUE;	//$ErrMsg = 'tes';
	 $form_name = $this->Prefix.'_form';
	 ...
	 $this->form_height = 175;
			...
			$getData = $this->sqlArray($this->sqlQuery("select * from refUser where id = '$idrefUser'"));
			foreach ($getData as $key => $value) {
	 			 $$key = $value;
	 		 }
			 $jenisrefUser = $type_refUser;
	 ...
	 //items ----------------------
	  $this->form_fields = array(
		...
		//tombol
		$this->form_menubawah =
			"<input type='button' class='btn btn-success' value='Simpan' onclick ='".$this->Prefix.".savePembayaranKomisi($idrefUser)' title='Simpan'>&nbsp&nbsp".
	...
	function broadcastEmail(){
	 ...
	 $this->form_height = 175;
			...
			$listEmail = implode(';',$_REQUEST['refUser_cb']);
	 ...
	 //items ----------------------
	  $this->form_fields = array(
		...
		//tombol
		$this->form_menubawah =
			"<input type='button' value='Simpan' onclick ='".$this->Prefix.".sendBroadCast($idrefUser)' title='Simpan'>&nbsp&nbsp".
	...
	function addTestimoni($idrefUser){
	 global $SensusTmp;
	 $cek = ''; $err=''; $content='';
	 $json = TRUE;	//$ErrMsg = 'tes';
	 $form_name = $this->Prefix.'_form';
	 ...
	 //items ----------------------
	  $this->form_fields = array(
		...
		//tombol
		$this->form_menubawah =
			"<input type='button' value='Simpan' class='btn btn-success' onclick ='".$this->Prefix.".saveTestimoni($idrefUser)' title='Simpan'>&nbsp&nbsp".
	 ...
	 $headerTable =
	  "<thead>
	   <tr>
  	   ...
  	   $Checkbox
			 <th class='th01' style='text-align:center;vertical-align:middle;width:70%;'>EMAIL</th>
	 ...
	 foreach ($_REQUEST as $key => $value) {
		...
		if(empty($jumlahData))$jumlahData =50;
          ...
          <div class='row' style='margin-top:5px !important;'>
            ...
            <div class='col-sm-3'>
							<input type='text' class='form-control form-control-sm' name='filterNama' id ='filterNama'  value='$filterNama'>
						</div>
          </div>
          <div class='row' style='margin-top:5px !important;'>
            <label class='col-sm-2 control-label control-label-sm' style='margin-top:6px;color: black;'>EMAIL</label>
            <div class='col-sm-3'>
							<input type='text' class='form-control form-control-sm' name='filterEmail' id ='filterEmail'  value='$filterEmail'>
						</div>
          </div>
          <div class='row' style='margin-top:5px !important;'>
	...
	function getDaftarOpsi($Mode=1){
		...
		if(!empty($filterNama)){
				$arrKondisi[] = "id_member in (select id from member where nama like '%$filterNama%')";
		}
		...
		$arrOrders = array();
		switch($filterUrut){
			case '1': $arrOrders[] = " type_refUser $Asc1 " ;break;
   ...
   $this->form_caption = 'DIREC TEAM';
	 ...
	 $getDataMember = $this->sqlArray($this->sqlQuery("select * from users where id = '$idMember'"));
	...
	function listDirectTeam($idMember){
			...
			$getDataMember = sqlQuery("select * from users where upline_level_1 = '".$idMember."' ");
			$no=1;
