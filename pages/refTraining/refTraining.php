...
class refTrainingObj  extends configClass{
	...
	var $PageTitle = 'refTraining';
	var $PageIcon = 'images/administrasi_ico.png';
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='refTraining.xls';
	var $namaModulCetak='refTraining';
	var $Cetak_Judul = 'refTraining';
	var $Cetak_Mode=2;
	...
	//get data -----------------
	 ...
	 $idplh = $_REQUEST[$this->Prefix.'_idplh'];
	   ...
	   if (empty($judulMateri)) {
	       $err = "Isi Judul ";
	   } elseif (empty($deskripsiMateri)) {
	       $err = "Isi Deskripsi";
	   } elseif (empty($statusMateri)) {
	       $err = "Pilih status ";
	   }
			...
			if($fmST == 0){
                ...
                'deskripsi_materi' => base64_encode($deskripsiMateri),
                ...
                'status' => $statusMateri,
							...
							$queryInsert = $this->sqlInsert('training',$dataInsert);
							$cek = $queryInsert;
				...
				if($err==''){
					...
					$queryUpdate = $this->sqlUpdate('training',$dataUpdate,"id = '".$idplh."'");
...
".
	  ...
	  if ($this->form_fmST==0) {
		$this->form_caption = 'Baru';
			...
			foreach ($getData as $key => $value) {
			 ...
			 $deskripsiMateri = base64_decode($deskripsi_materi);
		}
		...
		$this->form_fields = array(
			'judul' => array(
						...
						'labelWidth'=>3,
						'contentWidth'=>8,
						'value'=>"<input type='text' name = 'judulMateri' id = 'judulMateri' class='form-control form-control-sm' value='$judul_materi' >",
						 ),
			 'kategoriMateri' => array(
					 'label'=>'KATEGORI',
					 'labelWidth'=>3,
					 'contentWidth'=>2,
					 'value'=> cmbArray("kategoriMateri",$kategori,array(array("TRAINING","TRAINING"),array("PRODUK","PRODUK")),"-- KATEGORI --","class='form-control form-control-sm'"),
				),
				'deskripsi' => array(
							'label'=>'DESKRIPSI',
							'labelWidth'=>3,
							'contentWidth'=>8,
							// 'type' => 'manual',
							'value'=>"<div id ='deskripsiMateri'  style='height:250px;' class='quill-container ' >$deskripsiMateri</div>",
							 ),
			'sourceVideo' => array(
						'label'=>'SOURCE VIDEO',
						'labelWidth'=>3,
						'contentWidth'=>8,
						'value'=>"<input type='text' name = 'sourceVideo' id = 'sourceVideo'  class='form-control form-control-sm' value='$video_souce' >",
						 ),
			'durasiVideo' => array(
						'label'=>'DURASI VIDEO',
						'labelWidth'=>3,
						'contentWidth'=>2,
						'value'=>"<input type='text' name = 'durasiVideo' id = 'durasiVideo'  class='form-control form-control-sm' value='$durasi_video' >",
						 ),
			'Thumbnail' => array(
						'label'=>'THUMBNAIL VIDEO',
						'labelWidth'=>3,
						'contentWidth'=>8,
						'value'=>"<input type='text' name = 'thumbnailVideo' id = 'thumbnailVideo'  class='form-control form-control-sm' value='$thumbnail' >",
						 ),
			'urutanTraining' => array(
						'label'=>'URUTAN',
						'labelWidth'=>3,
						'contentWidth'=>1,
						'value'=>"<input type='text' name = 'urutanTraining' id = 'urutanTraining'  class='form-control form-control-sm' value='$urutan' >",
						 ),
			'wajibTonton' => array(
						'label'=>'WAJIB TONTON',
						'labelWidth'=>3,
						'contentWidth'=>2,
						'value'=>cmbArray("wajibTonton",$wajib_tonton,array(array("WAJIB","WAJIB"),array("TIDAK WAJIB","TIDAK WAJIB")),"-- WAJIB TONTON --","class='form-control form-control-sm'"),
						 ),
	 			'status' => array(
						'label'=>'STATUS',
						'labelWidth'=>3,
						'contentWidth'=>2,
						'value'=> cmbArray("statusMateri",$status,array(array("AKTIF","AKTIF"),array("TIDAK AKTIF","TIDAK AKTIF")),"-- STATUS --","class='form-control form-control-sm'"),
				 ),
			);
		//tombol
		$this->form_menubawah =
	...
	function bayarKomisi($idrefTraining){
	 global $SensusTmp;
	 $cek = ''; $err=''; $content='';
	 $json = TRUE;	//$ErrMsg = 'tes';
	 $form_name = $this->Prefix.'_form';
	 ...
	 $this->form_height = 175;
			...
			$getData = $this->sqlArray($this->sqlQuery("select * from refTraining where id = '$idrefTraining'"));
			foreach ($getData as $key => $value) {
	 			 $$key = $value;
	 		 }
			 $jenisrefTraining = $type_refTraining;
	 ...
	 //items ----------------------
	  $this->form_fields = array(
		...
		//tombol
		$this->form_menubawah =
			"<input type='button' class='btn btn-success' value='Simpan' onclick ='".$this->Prefix.".savePembayaranKomisi($idrefTraining)' title='Simpan'>&nbsp&nbsp".
	...
	function broadcastEmail(){
	 ...
	 $this->form_height = 175;
			...
			$listEmail = implode(';',$_REQUEST['refTraining_cb']);
	 ...
	 //items ----------------------
	  $this->form_fields = array(
		...
		//tombol
		$this->form_menubawah =
			"<input type='button' value='Simpan' onclick ='".$this->Prefix.".sendBroadCast($idrefTraining)' title='Simpan'>&nbsp&nbsp".
	...
	function addTestimoni($idrefTraining){
	 global $SensusTmp;
	 $cek = ''; $err=''; $content='';
	 $json = TRUE;	//$ErrMsg = 'tes';
	 $form_name = $this->Prefix.'_form';
	 ...
	 //items ----------------------
	  $this->form_fields = array(
		...
		//tombol
		$this->form_menubawah =
			"<input type='button' value='Simpan' class='btn btn-success' onclick ='".$this->Prefix.".saveTestimoni($idrefTraining)' title='Simpan'>&nbsp&nbsp".
	 ...
	 $headerTable =
	  "<thead>
	   <tr>
  	   ...
  	   $Checkbox
		   <th class='th01' style='text-align:center;vertical-align:middle;width:60%;'>JUDUL</th>
		   <th class='th01' style='text-align:center;vertical-align:middle;width:10%;'>KATEGORI</th>
		   <th class='th01' style='text-align:center;vertical-align:middle;width:10%;'>WAJIB TONTON</th>
		   <th class='th01' style='text-align:center;vertical-align:middle;width:10%;'>URUTAN</th>
		   <th class='th01' style='text-align:center;vertical-align:middle;width:10%;'>DURASI</th>
	 ...
	 foreach ($_REQUEST as $key => $value) {
		...
		if(empty($jumlahData))$jumlahData =50;
          ...
          <div class='row' style='margin-top:5px !important;'>
            <label class='col-sm-2 control-label control-label-sm' style='margin-top:6px;color: black;'>JUDUL</label>
            <div class='col-sm-3'>
							<input type='text' class='form-control form-control-sm' name='filterJudul' id ='filterJudul'  value='$filterJudul'>
						</div>
          </div>
          <div class='row' style='margin-top:5px !important;'>
            <label class='col-sm-2 control-label control-label-sm' style='margin-top:6px;color: black;'>KATEGORI</label>
            <div class='col-sm-2'>
							".cmbArray('filterKategori',$filterKategori,array(array("PRODUK","PRODUK"),array("TRAINING","TRAINING")),'-- KATEGORI --',"class='form-control form-control-sm'")."
						</div>
          </div>
	...
	function getDaftarOpsi($Mode=1){
		...
		$arrOrders = array();
		switch($filterUrut){
			case '1': $arrOrders[] = " type_refTraining $Asc1 " ;break;
   ...
   $this->form_caption = 'DIREC TEAM';
	 ...
	 $getDataMember = $this->sqlArray($this->sqlQuery("select * from training where id = '$idMember'"));
	...
	function listDirectTeam($idMember){
			...
			$getDataMember = sqlQuery("select * from training where upline_level_1 = '".$idMember."' ");
			$no=1;
