...
class refCopyWritingObj  extends configClass{
	...
	var $PageTitle = 'refCopyWriting';
	var $PageIcon = 'images/administrasi_ico.png';
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='refCopyWriting.xls';
	var $namaModulCetak='refCopyWriting';
	var $Cetak_Judul = 'refCopyWriting';
	var $Cetak_Mode=2;
	...
	//get data -----------------
	 ...
	 if (empty($judulCopyWriting)) {
			 $err = "Isi Judul";
	 } elseif (empty($isiKonten)) {
			 $err = "Isi Konten";
	 } elseif (empty($statusCopyWriting)) {
			 $err = "Pilih Funnel";
	 }
			...
			if($fmST == 0){
                ...
                'isi_content' => $isiKonten,
                'tanggal' => date("Y-m-d"),
                'status' => $statusCopyWriting,
							);
							$queryInsert = $this->sqlInsert('copy_writing',$dataInsert);
							$cek = $queryInsert;
				...
				if($err==''){
					...
					$queryUpdate = $this->sqlUpdate('copy_writing',$dataUpdate,"id = '".$idplh."'");
...
".
	  ...
	  if ($this->form_fmST==0) {
		...
		$this->form_fields = array(
			'judul' => array(
						...
						'labelWidth'=>2,
						'contentWidth'=>10,
						'value'=>"<input type='text' name = 'judulCopyWriting' id = 'judulCopyWriting' class='form-control form-control-sm' value='$judul' >",
						 ),
		 'isiKonten' => array(
	 				'label'=>"KONTEN",
	 				'labelWidth'=>2,
	 			...
	 			'status' => array(
						...
						'labelWidth'=>2,
						'contentWidth'=>10,
						'value'=> cmbArray("statusCopyWriting",$status,array(array("AKTIF","AKTIF"),array("TIDAK AKTIF","TIDAK AKTIF")),"-- STATUS --","class='form-control form-control-sm'"),
				 ),
			);
		//tombol
		$this->form_menubawah =
	...
	function bayarKomisi($idrefCopyWriting){
	 global $SensusTmp;
	 $cek = ''; $err=''; $content='';
	 $json = TRUE;	//$ErrMsg = 'tes';
	 $form_name = $this->Prefix.'_form';
	 ...
	 $this->form_height = 175;
			...
			$getData = $this->sqlArray($this->sqlQuery("select * from refCopyWriting where id = '$idrefCopyWriting'"));
			foreach ($getData as $key => $value) {
	 			 $$key = $value;
	 		 }
			 $jenisrefCopyWriting = $type_refCopyWriting;
	 ...
	 //items ----------------------
	  $this->form_fields = array(
		...
		//tombol
		$this->form_menubawah =
			"<input type='button' class='btn btn-success' value='Simpan' onclick ='".$this->Prefix.".savePembayaranKomisi($idrefCopyWriting)' title='Simpan'>&nbsp&nbsp".
	...
	function broadcastEmail(){
	 ...
	 $this->form_height = 175;
			...
			$listEmail = implode(';',$_REQUEST['refCopyWriting_cb']);
	 ...
	 //items ----------------------
	  $this->form_fields = array(
		...
		//tombol
		$this->form_menubawah =
			"<input type='button' value='Simpan' onclick ='".$this->Prefix.".sendBroadCast($idrefCopyWriting)' title='Simpan'>&nbsp&nbsp".
	...
	function addTestimoni($idrefCopyWriting){
	 global $SensusTmp;
	 $cek = ''; $err=''; $content='';
	 $json = TRUE;	//$ErrMsg = 'tes';
	 $form_name = $this->Prefix.'_form';
	 ...
	 //items ----------------------
	  $this->form_fields = array(
		...
		//tombol
		$this->form_menubawah =
			"<input type='button' value='Simpan' class='btn btn-success' onclick ='".$this->Prefix.".saveTestimoni($idrefCopyWriting)' title='Simpan'>&nbsp&nbsp".
	 ...
	 $headerTable =
	  "<thead>
	   <tr>
  	   ...
  	   $Checkbox
		   <th class='th01' style='text-align:center;vertical-align:middle;width:20%;'>JUDUL</th>
		   <th class='th01' style='text-align:center;vertical-align:middle;width:70%;'>ISI</th>
	...
	function getDaftarOpsi($Mode=1){
		...
		$arrOrders = array();
		switch($filterUrut){
			case '1': $arrOrders[] = " type_refCopyWriting $Asc1 " ;break;
   ...
   $this->form_caption = 'DIREC TEAM';
	 ...
	 $getDataMember = $this->sqlArray($this->sqlQuery("select * from copy_writing where id = '$idMember'"));
	...
	function listDirectTeam($idMember){
			...
			$getDataMember = sqlQuery("select * from copy_writing where upline_level_1 = '".$idMember."' ");
			$no=1;
